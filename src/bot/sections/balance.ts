...
export async function show_balance(chatId:number, userId:number) {
    await bot.sendMessage(chatId, `Saldo atual: R$ ${((await getBalance(userId)).toFixed(2)).replace('.', ',')}`);
}
    ...
    if(msg.text !== "🔄 VOLTAR AO MENU FINANCEIRO"){
        if(mode == "deposit"){
            if(validate_value(msg.text)){
                await bot.sendMessage(msg.chat.id, `o valor a ser depositado é R$ ${(parseFloat(msg.text.replace(',', '.')).toFixed(2)).replace('.', ',')}`);
...
if(msg.text !== "🔄 VOLTAR AO MENU FINANCEIRO"){
    if(mode == "withdrawal"){
        if(validate_value(msg.text)){
            await bot.sendMessage(msg.chat.id, `o valor a ser sacado é R$ ${(parseFloat(msg.text.replace(',', '.')).toFixed(2)).replace('.', ',')}`);
