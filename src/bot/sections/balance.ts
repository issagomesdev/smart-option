...
export async function show_balance(chatId:number, userId:number) {
    await bot.sendMessage(chatId, `Saldo atual: R$ ${(Math.floor((await getBalance(userId)) * 100) / 100).toString().replace('.', ',')}`);
}
    ...
    if(msg.text !== "🔄 VOLTAR AO MENU FINANCEIRO"){
        if(mode == "deposit"){
            if(validate_value(msg.text)){
                await bot.sendMessage(msg.chat.id, `o valor a ser depositado é R$ ${(Math.floor(parseFloat(msg.text.replace(',', '.')) * 100) / 100).toString().replace('.', ',')}`);
...
if(msg.text !== "🔄 VOLTAR AO MENU FINANCEIRO"){
    if(mode == "withdrawal"){
        if(validate_value(msg.text)){
            await bot.sendMessage(msg.chat.id, `o valor a ser sacado é R$ ${(Math.floor(parseFloat(msg.text.replace(',', '.')) * 100) / 100).toString().replace('.', ',')}`);
