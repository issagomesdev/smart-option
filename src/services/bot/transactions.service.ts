...
export class TransactionsService {
  ...
  static async balance(telegramId:number = null, earnings:boolean = true, user:any = null){
    try {
      ...
      const balance:any = (
        await conn.query(`SELECT * FROM balance WHERE user_id = '${user.user_id || user.id}' ${!earnings? "AND origin != 'earnings' AND MONTH(created_at) = MONTH(CURDATE())" : ""} ORDER BY created_at`)
      )[0];
